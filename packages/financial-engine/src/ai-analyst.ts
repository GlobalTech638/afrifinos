...
/**
 * Produces explainable facts without an LLM. This is the safe fallback and
 * reference implementation for the eventual provider-backed AI analyst.
 */
export function deriveFinancialFacts(snapshot: FinancialIntelligenceSnapshot): readonly FinancialFact[] {
  ...
  if (summary.savings.savingsRate !== null && summary.savings.savingsRate < 0) {
    facts.push(fact(
      "savings-negative",
      "savings",
      "Spending is currently higher than recorded income.",
      "critical",
      ["summary.savings.savingsRate"],
    ));
  }
  ...
  if (summary.debt.debtBurdenRatio !== null && summary.debt.debtBurdenRatio >= 0.4) {
    facts.push(fact(
      "debt-high-burden",
      "debt",
      "Debt service is consuming a high share of recorded income.",
      "warning",
      ["summary.debt.debtBurdenRatio"],
    ));
  }
  ...
  if (forecast.minimumProjectedBalanceMinor < 0n) {
    facts.push(fact(
      ...
      "forecast",
      "The current forecast projects a negative cash balance within the selected horizon.",
      "critical",
      ["forecast.minimumProjectedBalanceMinor", "forecast.runwayDays"],
    ));
  }
  ...
  for (const anomaly of snapshot.temporal.anomalies.slice(0, 5)) {
    facts.push(fact(
      ...
      "anomaly",
      `Transaction ${anomaly.transactionId} is unusually large relative to its baseline.\`,
      "warning",
      [`temporal.anomalies.${anomaly.transactionId}`],
    ));
  }
  ...
  for (const recurring of snapshot.temporal.recurring.slice(0, 10)) {
    if (recurring.type === "expense") {
      facts.push(fact(
        ...
        "recurring",
        `A recurring expense is detected approximately every ${recurring.averageIntervalDays} days.`,
        "info",
        [`temporal.recurring.${recurring.key}`],
      ));
    }
  }
  ...
  if (facts.length === 0) {
    facts.push(fact(
      "financial-state-no-alerts",
      "health",
      "No high-priority financial alerts were detected by the current deterministic rules.",
      "info",
      ["summary.healthScore"],
    ));
  }
      ...
      return {
        ...
        advice: facts
          ...
          .map((item) => ({
            ...
            title: item.category === "debt" ? "Review debt burden" : item.category === "forecast" ? "Protect upcoming cash" : "Review financial alert",
